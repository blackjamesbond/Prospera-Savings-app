...
const AdminDashboard: React.FC = () => {
  ...
  return (
    <div className="space-y-4 animate-in fade-in duration-700 pb-20">
      <div className="relative p-5 bg-prospera-accent/5 border border-prospera-accent/10 rounded-2xl overflow-hidden terminal-grid">
         <div className="absolute top-0 right-0 p-6 opacity-10"><Activity className="w-24 h-24 text-prospera-accent animate-pulse" /></div>
         <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            <div className="space-y-1">
               <div className="flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full bg-prospera-accent animate-ping" /><h2 className="text-xl font-black dark:text-white text-gray-900">Admin Control Panel</h2></div>
               <p className="text-prospera-gray font-black uppercase tracking-[0.3em] text-[8px]">Circle Integrity Overview</p>
            </div>
            <div className="flex gap-2">
               <div className="px-3 py-1.5 bg-white dark:bg-prospera-dark rounded-lg border border-gray-100 dark:border-white/5 shadow-lg text-center"><p className="text-[7px] text-prospera-gray font-black uppercase mb-0.5">Terminal</p><p className="text-xs font-black text-prospera-accent">Online</p></div>
            </div>
         </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        {dashboardConfig.showLogs && (
          <div className={`${dashboardConfig.showAnalytics ? 'lg:col-span-1' : 'lg:col-span-3'} p-5 rounded-2xl bg-white dark:bg-prospera-dark border border-gray-100 dark:border-white/5 shadow-xl flex flex-col`}>
            ...
            <div className="space-y-2.5 flex-1 overflow-y-auto max-h-[220px] no-scrollbar">
              {notifications.length > 0 ? notifications.map(n => (<div key={n.id} className="p-3 rounded-xl bg-gray-50 dark:bg-prospera-darkest/50 border-l-2 border-prospera-accent group hover:bg-prospera-accent/5 transition-colors"><p className="text-[8px] font-black dark:text-white text-gray-900 uppercase mb-0.5">{n.title}</p><p className="text-[9px] text-prospera-gray line-clamp-2 leading-tight">{n.message}</p></div>)) : <div className="flex-1 flex flex-col items-center justify-center opacity-20 py-10"><Terminal className="w-8 h-8 text-prospera-gray mb-2" /><p className="text-[7px] font-black uppercase tracking-widest">Feed Null</p></div>}
            </div>
          </div>
        )}
      </div>
      ...
      {dashboardConfig.showRecentTransactions && (
        <div className="p-5 rounded-2xl bg-white dark:bg-prospera-dark border border-gray-100 dark:border-white/5 shadow-xl">
          <h2 className="text-sm font-black dark:text-white text-gray-900 mb-4 tracking-widest uppercase">Validated Ledger Entries</h2>
