    ...
    <div className="space-y-6 animate-in fade-in duration-700 pb-20">
      <div className="relative p-6 bg-prospera-accent rounded-[2.5rem] shadow-2xl overflow-hidden group">
         ...
         <div className="relative z-10 space-y-3">
            <div className="flex items-center gap-2"><ShieldCheck className="w-4 h-4 text-white" /><span className="text-[8px] font-black uppercase tracking-[0.4em] text-white/80">Identity Verified • Standing: Prime</span></div>
            <h2 className="text-3xl font-black tracking-tighter text-white">Welcome, {currentUser.name}.</h2>
            <p className="text-sm text-white/80 font-medium leading-relaxed max-w-xl">
               Personal equity verified at <span className="font-black text-white">{preferences.currency} {myTotal.toLocaleString()}</span>. 
               Group protocol rank: <span className="font-black text-white">#{currentUser.rank}</span>.
            </p>
         </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <DashboardCard title="Personal Equity" value={`${preferences.currency} ${myTotal.toLocaleString()}`} subtitle="Verified Contribution" icon={TrendingUp} />
        {dashboardConfig.showTarget && <DashboardCard title="Goal Proximity" value={`${Math.round(progress)}%`} subtitle={`${preferences.currency} ${remainingToTarget.toLocaleString()} left`} icon={Target} />}
        <DashboardCard title="Alerts" value={notifications.filter(n => !n.read && (!n.recipientId || n.recipientId === currentUser.id)).length} icon={Bell} subtitle="Unread Packets" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {dashboardConfig.showTarget && (
          <div className="p-6 rounded-[2rem] bg-white dark:bg-prospera-dark border border-gray-100 dark:border-white/5 shadow-2xl flex flex-col justify-between terminal-grid">
            <div>
               <div className="flex items-center justify-between mb-4"><h2 className="text-lg font-black dark:text-white text-gray-900 tracking-tight">Active Objective</h2><div className="p-2 bg-prospera-accent/10 rounded-xl"><Zap className="text-prospera-accent w-4 h-4" /></div></div>
            ...
            <div className="space-y-3 mt-8">
              <div className="flex justify-between items-end"><span className="text-[8px] font-black uppercase tracking-widest text-prospera-gray">Accumulation</span><span className="text-xl font-black text-prospera-accent tracking-tighter">{Math.round(progress)}%</span></div>
        ...
        {dashboardConfig.showLogs && (
          <div className="p-6 rounded-[2rem] bg-white dark:bg-prospera-dark border border-gray-100 dark:border-white/5 shadow-2xl">
            <h2 className="text-lg font-black mb-4 dark:text-white text-gray-900 tracking-tight">System Feed</h2>
            <div className="space-y-3">
              ...
              {notifications.filter(n => !n.recipientId || n.recipientId === currentUser.id).length === 0 && (
                <p className="text-[10px] text-prospera-gray font-bold italic py-8 text-center uppercase tracking-widest">Feed Clear</p>
              )}
            </div>
          </div>
        )}
      ...
      {dashboardConfig.showRecentTransactions && (
        <div className="p-6 rounded-[2rem] bg-white dark:bg-prospera-dark border border-gray-100 dark:border-white/5 shadow-2xl">
          <h2 className="text-lg font-black mb-4 dark:text-white text-gray-900 tracking-tight">Verified Logs</h2>
